...
macOS-style screen-recording permission needed), captured by window ID
(~xwininfo~ + ~xwd -id~, not ~xwd -root~ -- the latter captures the
whole virtual display, leaving a black band where the window doesn't
fill it).
