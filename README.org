...
[[file:lean4-workshop.el][lean4-workshop.el]] gives every command above a key under ~C-c l~
and works with or without ~lean4-mode~. Load it, then ~M-x l4w/setup~:

#+BEGIN_SRC emacs-lisp
(load-file "~/ghq/github.com/jwalsh/lean4-workshop/lean4-workshop.el")
(l4w/setup)
#+END_SRC
...
| ~C-c l s~ | ~l4w/solution~          | toggle exercise and its solution                        |
...
| ~C-c l v~ | ~l4w/eval~              | ~#eval~ an expression with the buffer above point in scope |
...
The mode line shows the number of ~sorry~ left in the buffer. ~l4w/eval~ and
~l4w/check-expr~ are the debugger when there is no LSP: they append the
command to a copy of the buffer and run ~lean~, so the expression sees every
definition above point, ~sorry~ included.

With [[https://github.com/leanprover-community/lean4-mode][lean4-mode]] installed
(~M-x l4w/install-lean4-mode~ does it via ~package-vc~), ~l4w/setup~ also
starts ~lsp-mode~ for goals and hover. ~flycheck~ gets a ~lean4-standalone~
checker that works on single files with no Lake project.
...
See the [[*Emacs][Emacs]] section above for the repo's own ~lean4-workshop.el~.
For the language mode itself:
